...
// - The for...in loop will iterate over both the instance properties (like 'sound') 
//   and any properties inherited from the prototype (like 'name' from Animal).
// - This means you might see properties from parent classes or from the object's 
//   prototype chain, not just those directly defined on the instance
...
//Prototype Chain
// - The for...in loop not only iterates over an object's own enumerable properties 
//   but also over those properties down its prototype chain
// - This can result in unexpected properties being iterated over if the object's
//   prototype has enumerable properties
// - Iterating over the prototype chain can lead to unexpected behavior if not handled
//   carefully, as it may include properties you do not intend to access
...
// - The `propertyIsEnumerable()` method can be used to check if a specific property is
//   enumerable on an object
console.log(dog.propertyIsEnumerable("sound")); //Outputs: true



//Error Case for for...of
// - Non-iterable objects (like regular objects) will cause an error if used with for...of
// - Attempting to use a for...of loop with a non-iterable object will throw a `TypeError`
try {
...
// - Ensure that the object you use with for...of is iterable (e.g., arrays, strings, maps)
// - If you need to iterate over properties of a non-iterable object, consider using 
//   for...in instead
...
// - Suitable for iterating over the properties of objects and the indices of arrays
