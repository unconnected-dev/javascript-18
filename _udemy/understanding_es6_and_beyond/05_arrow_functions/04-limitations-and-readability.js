...
// Arrow functions can make code more concise, but this brevity can sometimes 
// reduce readability.
// When functions become more complex or heavily nested, arrow functions may 
// obscure the logic.
...
// The function's structure and logic are easier to follow, making it more 
// readable, especially for those unfamiliar with arrow functions.
...
let me = {
    ...
    greet2: () => {
        ...
        // `this` does not refer to the `me` object, but to the global object 
...
// Memory Usage: 
// - Defining methods like `greet` inside the constructor using an arrow 
//   function means each instance of `Person` gets its own copy of 
//   `greet`. 
...
// Arrow Functions:
// - Arrow functions are concise but can reduce readability, especially in 
//   complex or nested cases.
// - Arrow functions do not bind their own `this`, making them unsuitable for
//   object methods where `this` should refer to the object.
// - Arrow functions are more suitable for callbacks or functions where you 
//   want to preserve the `this` context from the outer scope.
// - Be cautious when using arrow functions in classes, as they increase memory
//   usage and break the traditional prototype chain.