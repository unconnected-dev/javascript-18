...
// functions. In the following example, `phrase` is a function that uses 
// `fname` and `lname` in its output.
...
// The key difference between `greet3` and `greet4` is that in `greet3`, 
// `phrase` is a function, and the function only runs after all parameters have
// been initialized.
// In `greet4`, `phrase` is a string literal that tries to use `lname` 
// immediately, but `lname` has not been initialized yet, causing an error.

// This highlights that when using parameters as part of default values, the 
// order of parameters matters if they are used before being initialized.