...
if (a == b) {
    // `var` is function-scoped or globally scoped, so `myVar` is accessible 
    // outside of this block as well.
    var myVar = 1;
    ...
    // Not accessible: `myOtherVar` is block-scoped to the inner `if` block and 
    // cannot be accessed here. It would cause an error.
    // console.log(myOtherVar);


    // Accessible: `myLet` is still accessible here since we are in the same block 
