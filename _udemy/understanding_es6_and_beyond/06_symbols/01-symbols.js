...
// This method could overwrite the original `greet` method directly on the 
// object.
if(false){
    tony.greet = function() { };
    console.log(tony.greet());
...
// Symbols are useful for creating unique property keys that cannot be 
...
console.log(tony2.greet());
console.log(tony2[grt]());  
...
// Well-Known Symbols:
// JavaScript has several built-in symbols that the engine uses for specific 
// tasks. These are known as "well-known symbols." 
// For example:
// - `Symbol.iterator`: Used to define the default iterator for an object.
// - `Symbol.toStringTag`: Used to create a custom string description of an
//   object when `toString` is called.



// Symbols: 
// - Unique, immutable primitive values that are ideal for creating unique 
//   property keys.
...
// Global Symbols:
// - Symbols can be shared across different parts of a program using 
//   Symbol.for, stored in a global registry.
