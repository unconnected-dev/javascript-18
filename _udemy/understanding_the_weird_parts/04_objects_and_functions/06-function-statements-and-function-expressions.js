...
if(false){
    ...
    //Assignment operator `=` assigns the value 3 to `a` (expression result is 3)
    a = 3;

    //Addition `1 + 2` is an expression that evaluates to 3
    1 + 2;

    //Object literal: Creating and assigning an object to `a` is also an expression
    a = { greeting: `Hi` }
...
//Function Statement:
//This is a function declaration or statement. It is hoisted, meaning you can 
//call `greet()` before its definition
greet();
function greet(){
    console.log(`Hi`);
}
...
//  - Function statements are hoisted, meaning they can be called before their
//    declaration
//  - Function expressions are not hoisted; only the variable is hoisted, and
//    it's initialized as `undefined` during the creation phase
...
//You can also create a function on the fly (a function expression) and pass it
//as an argument
log(function(){
    console.log(`hi`);
});