...
//In statically typed languages (e.g., Java or C#), you must declare the type of
//a variable explicitly, and the type cannot change
...
//Objects, on the other hand, are collections of key-value pairs and can hold more
//complex data



//Undefined
//`undefined` represents the absence of a value. It is the default value assigned 
//to variables that are declared but not yet initialized

//`undefined` is automatically assigned by the JavaScript engine when a variable
...
//Null
//`null` is an explicit assignment to indicate the intentional absence of a value.
//It's useful when you want to reset a variable or signal that the variable has no
//value

//`null` is different from `undefined` in that it is explicitly set by the developer,
//whereas `undefined` is set by JavaScript itself. Use `null` when you want to 
//intentionally denote "no value"
if(false){
    let anotherVar = null;
...
//Boolean
//A `boolean` is a logical value that can either be `true` or `false`. They are often
//used in conditions and logical expressions to control the flow of code
let isUserLoggedIn = false;  


//Number
//Unlike many programming languages, JavaScript does not have separate types for 
//integers and floating-point numbers; it uses the `number` type for both
let myNumber = 42;
let myFloat = 3.14;

//JavaScript's `number` type is a floating-point type, meaning that even if you're 
//working with integers, calculations may produce unexpected results due to how 
