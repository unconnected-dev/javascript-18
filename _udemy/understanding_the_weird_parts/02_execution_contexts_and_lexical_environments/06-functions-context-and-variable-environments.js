...
function a(){
    ...
    //This `myVar` is local to the function `a`, separate from `myVar` in `b` or 
    //the global scope
    var myVar = 2;  
...
// - `undefined`: Logs the `myVar` inside `b()` which is declared but not initialized
...
// - Each variable belongs to its own scope, which is defined by where the variable is
//   declared
// - In JavaScript, every function creates its own execution context and scope when 
//   invoked
// - Variables declared inside a function (with `var`, `let`, or `const`) are local to 
//   that function's scope
// - The same variable name can exist in different scopes and each scope will have its 
