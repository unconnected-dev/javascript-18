...
console.log(`${a}`);  //`a` exists, but its value is `undefined` due to hoisting
...
console.log(`${a}`);  //Now logs 'Hello World' since `a` has been initialized



//The phenomenon where functions and variables are accessible before their declaration
//in the code is called 'hoisting'. However, hoisting behaves differently for `var`, 
//`let`, and `const`. Using `let` or `const` would throw a reference error if accessed
//before declaration
if(false){
...
//When JavaScript code is executed, the execution context is created in two phases:

//Creation Phase:
// - Creates the global object (`window` in browsers, `global` in Node.js)
// - Sets up the special `this` keyword
// - Defines the outer environment (the global context in the case of global code)

// - During this phase, the JavaScript engine scans the code and sets up memory space 
//   for variables and functions
// - Variables declared with `var` are hoisted and initialized with the value `undefined`
// - Functions are hoisted in their entirety, meaning the function code is already 
...
// - The function `b`, on the other hand, is fully hoisted and available for use even
...
// - The engine starts executing the code using the memory space set up during the 
//   creation phase
...
// - Variables declared with `let` and `const` are hoisted but not initialized until
//   the line of code where they are declared
// - Accessing them before declaration results in a ReferenceError (the "temporal dead zone")
